export const LOGIN = "/login";
export const SIGNUP = "/signup";
export const CREATE_PASSWORD = "/password";
export const CREATE_COMPANY_PASSWORD = "/company/password";
export const CREATE_COMPANY_PROFILE = "/company/create-profile";
export const CREATE_COMPANY_OTP = "/company/otp";
export const CREATE_PROFILE = "/create-profile";
export const DOCUMENT = "/document";
export const OTP = "/otp";

export const HOME = "/home";
export const MYPROFILE = "/home/myprofile";
export const USERS = "/home/users";
export const TRANSACTIONS = "/home/transactions";
export const SECURITY = "/home/security";
export const ELIGIBILITY = "home/eligibility";
export const ACTIVITY = "/home/activity";
